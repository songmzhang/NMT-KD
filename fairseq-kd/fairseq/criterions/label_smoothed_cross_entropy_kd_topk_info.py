...
@register_criterion(
    "label_smoothed_cross_entropy_kd_topk_info", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_kd_loss(self, student_logits, teacher_logits, sample=None):
        ...
        teacher_topk_probs, teacher_topk_preds = torch.topk(teacher_probs, k=self.topk, dim=-1)
        lprobs = utils.log_softmax(student_logits, -1)
        topk_kd_loss = -lprobs.gather(dim=-1, index=teacher_topk_preds)
        topk_kd_loss = topk_kd_loss * teacher_topk_probs
        topk_kd_loss = topk_kd_loss.sum(-1)
        if sample is not None:
            pad_mask = sample["target"].eq(self.padding_idx)
            topk_kd_loss.masked_fill_(pad_mask, 0.0)
        topk_kd_loss = topk_kd_loss.sum()
        return topk_kd_loss
