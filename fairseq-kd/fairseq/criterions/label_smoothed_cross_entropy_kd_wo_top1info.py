...
@register_criterion(
    "label_smoothed_cross_entropy_kd_without_top1", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_kd_loss(self, student_logits, teacher_logits, sample=None):
        ...
        max_teacher_probs, max_teacher_targets = teacher_probs.max(-1, keepdim=True)
        teacher_probs = teacher_probs.scatter_(-1, max_teacher_targets, torch.zeros_like(max_teacher_probs))
        teacher_probs = teacher_probs.scatter_add_(-1, sample["target"].unsqueeze(-1), max_teacher_probs)
