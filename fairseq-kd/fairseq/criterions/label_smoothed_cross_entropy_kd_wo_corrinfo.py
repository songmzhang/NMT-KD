...
@register_criterion(
    "label_smoothed_cross_entropy_kd_without_correlation", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_kd_loss(self, student_logits, teacher_logits, sample=None):
        teacher_target = teacher_logits.argmax(-1, keepdim=True)
        teacher_max_probs = utils.softmax(teacher_logits, -1).max(-1, keepdim=True)[0]
        lprobs = utils.log_softmax(student_logits, -1)
        argmax_kd_loss = -lprobs.gather(dim=-1, index=teacher_target)
        argmax_kd_loss = argmax_kd_loss * teacher_max_probs
        argmax_kd_loss = argmax_kd_loss.squeeze(-1)
        if sample is not None:
            pad_mask = sample["target"].eq(self.padding_idx)
            argmax_kd_loss.masked_fill_(pad_mask, 0.0)
        argmax_kd_loss = argmax_kd_loss.sum()
        return argmax_kd_loss
