...
class TransformerEncoderLayerBase(nn.Module):
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(self, cfg, return_fc=False):
        ...
        self.self_attn_residual = admin_torch.as_module(
            cfg.encoder.layers * 2, 
            output_change_scale=cfg.encoder.output_change_scale,
            as_parameter=cfg.encoder.as_parameter,
            embed_dim=self.embed_dim
        )
        self.ffn_residual = admin_torch.as_module(
            cfg.encoder.layers * 2, 
            output_change_scale=cfg.encoder.output_change_scale,
            as_parameter=cfg.encoder.as_parameter,
            embed_dim=self.embed_dim
    ...
    def forward(
        ...
        x = self.self_attn_residual(residual, x)
        ...
        x = self.dropout_module(x)
        x = self.ffn_residual(residual, x)
        if not self.normalize_before:
            x = self.final_layer_norm(x)
...
class TransformerDecoderLayerBase(nn.Module):
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def __init__(
        ...
        self.self_attn_residual = admin_torch.as_module(
            cfg.decoder.layers * 3, 
            output_change_scale=cfg.decoder.output_change_scale,
            as_parameter=cfg.decoder.as_parameter,
            embed_dim=self.embed_dim
        )
        if not no_encoder_attn:
            self.encoder_attn_residual = admin_torch.as_module(
                cfg.decoder.layers * 3, 
                output_change_scale=cfg.decoder.output_change_scale,
                as_parameter=cfg.decoder.as_parameter,
                embed_dim=self.embed_dim
    ...
    def forward(
        ...
        if self.encoder_attn is not None and encoder_out is not None:
            ...
            x = self.dropout_module(x)
            x = self.encoder_attn_residual(residual, x)
            if not self.normalize_before:
                x = self.encoder_attn_layer_norm(x)
